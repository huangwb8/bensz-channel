#!/usr/bin/env python3
"""
env_init.py - 快速生成 bensz-channel-devtools 配置文件
...
def generate_env_content(url: str, key: str) -> str:
    """生成 .env 文件内容"""
    return f"""# bensz-channel-devtools 配置文件
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="快速生成 bensz-channel-devtools 配置文件",
...
示例：
  ...
  python3 scripts/env_init.py --path /path/to/.env
    ...
    if not api_key:
        print("=" * 60)
        print("bensz-channel-devtools 配置向导")
        print("=" * 60)
        print("\n请按照以下步骤获取 API 密钥：")
