...
- 文章发布通知现默认走队列异步发送，避免文章创建请求被收件人筛选和 SMTP 投递阻塞；SMTP 传输默认超时为 `10` 秒，可通过 `MAIL_TIMEOUT` 覆盖
...
- 管理员后台与 DevTools API 在标题主要为中文且未显式提供 slug 时，都会自动回退到 `article-xxxxxxxx` 随机 slug，避免空 slug 或重复提交时的异常
- 首页会优先展示置顶文章
- "精华"频道会聚合所有精华文章
...
- `POST /api/vibe/articles` 现支持可选请求头 `X-Idempotency-Key`
- 同一 API Key、同一幂等键、同一请求内容会返回同一篇文章，避免网络抖动或终端重试时重复发文
- 若同一幂等键被用于不同请求内容，服务端会返回 `409 idempotency_key_conflict`
