...
- 可保存 CDN 草稿配置，但保存后不会立刻影响线上
- 可点击 `应用 CDN` / `停止 CDN` 手动控制启用状态
- 可测试对象存储连接，并把具体失败原因写入工作日志
- 可查看同步差异、立即同步、清空远程文件
- 可查看最近 50 次工作日志
...
7. 点击 `保存 CDN 设置`
8. 先点击 `测试连接`，确认工作日志中没有错误
9. 再点击 `应用 CDN`
10. 如需立即推送资源，可再点击 `立即同步`
...
- 首次全量同步后，再开启 `构建后自动同步`
- 若测试失败，优先查看后台 `工作日志` 中的异常类型、异常原因与配置摘要
- 在 Docker 重部署后，通过 `./scripts/test/docker-smoke.sh` 与后台工作日志确认资源链接是否正确
