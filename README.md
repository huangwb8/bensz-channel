...
- 管理员登录后可进入“订阅设置”，在原有订阅偏好之外维护全站 SMTP 服务器、端口、账号、密钥与发件人信息，并可直接点击“测试 SMTP”按钮验证当前表单配置是否可正常投递
...
- 管理员可在同一页面测试当前 SMTP 表单配置；测试不会保存配置，邮件会发到“测试收件邮箱”字段指定的地址，便于验证当前表单中的 SMTP 服务器、端口、认证与发件人设置
