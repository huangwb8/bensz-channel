...
- **管理员后台**：默认预置 1 位管理员，可管理频道、文章与用户资料/角色
- **成员登录体系**：基于 Better Auth 的邮箱验证码、手机号验证码，以及微信/QQ 演示扫码登录
...
- 邮箱验证码：在登录页填写任意邮箱，例如 `member@example.com`
- Docker 审查环境默认不在页面展示验证码，请到 Mailpit 查看邮件验证码
- 手机验证码链路仍保留，但未接入真实短信网关；开发/测试环境可通过预览码或日志验证
