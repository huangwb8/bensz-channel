...
class ArticleTocTest extends TestCase
{
    ...
    public function test_article_page_renders_toc_and_numbered_headings(): void
    {
        ...
        $this->get(route('articles.show', [$channel, $article]))
            ...
            ->assertSee('lg:grid lg:grid-cols-[minmax(0,1fr)_18rem] lg:items-start lg:gap-8', false)
            ->assertSee('article-toc-mobile', false)
            ->assertSee('article-toc-mobile-list', false)
            ->assertSee('article-toc-link-mobile', false)
            ->assertSee('article-toc-desktop', false)
            ->assertSee('article-toc-desktop hidden lg:block lg:sticky lg:top-24 lg:self-start', false)
            ->assertSee('article-toc-panel rounded-2xl border border-gray-200 bg-gray-50 p-4', false)
            ->assertSee('data-article-toc', false)
            ->assertSee('aria-label="文章目录导航"', false)
            ->assertSee('data-toc-collapsible="true"', false)
            ->assertSee('data-toc-toggle', false)
            ->assertSee('data-toc-branch', false)
            ->assertSee('article-toc-toggle', false)
